    ...
    <div class="main-content d-flex" style="gap:20px;">
        ...
        <div class="pet-list-wrapper">
            <div class="pet-list" id="petList">
                ...
                <div class="pet-item d-flex align-items-center justify-content-between p-2 mb-2 border rounded"
                    data-state="<?= htmlspecialchars($pet['status']) ?>"
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div class="pet-info">
                            <h5 class="pet-name mb-1"><?= htmlspecialchars($pet["name"]) ?></h5>
                            <p class="mb-0"><strong>Giới tính:</strong> <?= htmlspecialchars($pet["gender"]) ?></p>
                            <p class="mb-0"><strong>Trạng thái:</strong> <?= htmlspecialchars($pet["status"]) ?></p>
        ...
        <?php
            $validPets = array_filter($pets, function ($pet) {
                return in_array($pet['status'], ['available', 'reserved']);
            });
            // ====== ĐẾM TRẠNG THÁI PET ======
            $availableCount = 0;
            $reservedCount = 0;
            ...
            foreach ($validPets as $pet) {
                if ($pet["status"] === "available") $availableCount++;
                else if ($pet["status"] === "reserved") $reservedCount++;
            }
